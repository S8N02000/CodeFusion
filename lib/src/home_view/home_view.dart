...
class HomeViewState extends ConsumerState<HomeView> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: _selectedDirectory.isNotEmpty ? Row(
      mainAxisSize: MainAxisSize.min,
      children: [Consumer(builder: (context, ref, _) {
        final folderMetadataAsyncValue = ref.watch(
            folderSvgIconMetadataLoaderProvider);
        return folderMetadataAsyncValue.when(data: (folderSvgIconMetadata) =>
            ElevatedButton(onPressed: _addDirectory, child: Row(children: [
      ...
      actions: [IconButton(icon: const Icon(Icons.refresh), // Icône refresh
        onPressed: () {
      ...
      body: Column(children: [
        Expanded(child: shouldShowPickDirectory
            ? Center(child: ElevatedButton(
          onPressed: _addDirectory, child: const Text("Pick Directory"),),)
            : fileMetadata.when(data: (fileSvgIconMetadata) =>
            folderMetadata.when(data: (folderSvgIconMetadata) =>
                files.when(data: (fileList) =>
                    FileListPanel(files: fileList ?? [],
          ...
          Padding(padding: const EdgeInsets.all(8.0),
            child: ElevatedButton(onPressed: ref
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                _isCopied ? const Icon(Icons.check, size: 16.0) : const Icon(
                    Icons.content_copy, size: 16.0),
                const SizedBox(width: 8),
                _isCopied ? const Text('Copied!') : _isLoading ? const SizedBox(
                  width: 16,
                  height: 16,
                  child: CircularProgressIndicator(strokeWidth: 2),) : Text(
      ...
      if (fileEntity == FileSystemEntityType.file) {
        try {
          ...
          String displayPath = settingsController.pathOption == PathOption.full
              ...
              : path.relative(
              filePath, from: _selectedDirectory); // Or the relative path
